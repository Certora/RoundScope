...
contract YieldSharingUnitTest is Fixture {
    ...
    function testAccrueProfitOnlySavingRecipients() public {
        ...
        assertEq(
            iusd.balanceOf(address(siusd)) + yieldSharing.vesting(),
            1020e18,
            "Error: iUSD balance of siusd should be increase after accrue"
        );
    ...
    function testAccrueProfitOnlylockingRecipients() public {
        ...
        assertEq(
            iusd.balanceOf(address(siusd)) + yieldSharing.vesting(),
            0,
            "Error: iUSD balance of siusd should be 0 after accrue"
        );
        assertEq(
    ...
    function testAccrueProfitSavingsAndlockingRecipients() public {
        ...
        assertEq(
            iusd.balanceOf(address(siusd)) + yieldSharing.vesting(),
            1000e18 + 20e18,
            "Error: iUSD balance of siusd should increase after accrue"
        ); // 10$ to staking
        assertEq(
            lockingController.totalBalance(),
            1024e18,
    ...
    function testAccrueLossesSlashOnlySavings() public {
        ...
        assertEq(
            iusd.balanceOf(address(siusd)), 1000e18 - 10e18, "Error: iUSD balance of siusd should decrease after accrue"
        ); // 5$ slash on staking
    }
    ...
    function testAccrueLossesSlashlockingFirst() public {
        ...
        assertEq(
            iusd.balanceOf(address(siusd)) + yieldSharing.vesting(),
            1020e18,
            "Error: iUSD balance of siusd should not change after accrue"
        ); // no slash on savings
    }
    ...
    function testAccrueLossesSlashlockingAndSavingFirst() public {
        ...
        assertEq(iusd.balanceOf(address(siusd)), 1000e18 - 176e18); // 176 slashed
    ...
    function testAccrueLossesSlashlockingAndSavingFirstThenUpdatePeg() public {
        ...
        // full vesting of the liquid rewards
        vm.warp(block.timestamp + yieldSharing.interpolationDuration());
        yieldSharing.accrue();
    ...
    function testSetLiquidReturnMultiplier() public {
        ...
        uint256 lockingBalanceAfter = lockingController.totalBalance();
        uint256 liquidRewards = liquidBalanceAfter - liquidBalanceBefore + yieldSharing.vesting();
    ...
    function testSetInterpolationDuration() public {
        uint256 duration = yieldSharing.interpolationDuration();

        vm.expectRevert("UNAUTHORIZED");
        yieldSharing.setInterpolationDuration(duration * 2);

        vm.prank(parametersAddress);
        yieldSharing.setInterpolationDuration(duration * 2);

        assertEq(yieldSharing.interpolationDuration(), duration * 2);
    }
    ...
    function testTargetIlliquidRatio() public {
        ...
        uint256 liquidBalanceAfter = iusd.balanceOf(address(siusd)) + yieldSharing.vesting();
        uint256 lockingBalanceAfter = lockingController.totalBalance();
    ...
    function testTryManipulateLiquidIlliquidRatio() public {
        ...
        vm.startPrank(carol);
        {
            ...
            yieldSharing.accrue();
            siusd.approve(address(gateway), 2_000_000e18);
            gateway.unstake(address(carol), 2_000_000e18);
        }
        vm.stopPrank();

        // if manipulation were successful, most rewards should
        // have gone to the liquid side instead of the 10/12 split.
        assertEq(iusd.balanceOf(address(siusd)) + yieldSharing.vesting(), 1000e18 + 20e18); // +10$ to staking
    ...
    function testInterpolationRewards() public {
        uint256 interpolationDuration = yieldSharing.interpolationDuration();

        // alice deposits 100$ and stakes
        vm.startPrank(alice);
        {
        ...
        assertEq(siusd.previewRedeem(siusd.balanceOf(alice)), 200e18);

        // simulate 10$ profit
        farm1.mockProfit(10e6);
        yieldSharing.accrue();

        assertEq(yieldSharing.vesting(), 20e18);
        assertEq(siusd.previewRedeem(siusd.balanceOf(alice)), 200e18);

        // go to 50% of the interpolation duration
        vm.warp(block.timestamp + interpolationDuration / 2);

        assertEq(yieldSharing.vesting(), 20e18);
        // some wei of rounding can happen because the vesting rate
        // is per second and it is rounded down
        uint256 maxRoundingErrorWei = interpolationDuration / 2;
        assertApproxEqAbs(yieldSharing.vested(), 10e18, maxRoundingErrorWei);

        // bob deposits 105$ and stakes
        vm.startPrank(bob);
        {
            usdc.mint(bob, 105e6);
            usdc.approve(address(gateway), 105e6);
            gateway.mintAndStake(bob, 105e6);
        }
        vm.stopPrank();

        // vested yield should be accrued
        assertApproxEqAbs(siusd.previewRedeem(siusd.balanceOf(alice)), 210e18, maxRoundingErrorWei);
        assertApproxEqAbs(siusd.previewRedeem(siusd.balanceOf(bob)), 210e18, 1);

        assertApproxEqAbs(yieldSharing.vesting(), 10e18, maxRoundingErrorWei);
        assertApproxEqAbs(yieldSharing.vested(), 0, maxRoundingErrorWei);

        // go to 75% of the interpolation duration
        vm.warp(block.timestamp + interpolationDuration / 4);
        maxRoundingErrorWei += interpolationDuration / 4;
        yieldSharing.distributeInterpolationRewards();

        assertApproxEqAbs(siusd.previewRedeem(siusd.balanceOf(alice)), 212.5e18, maxRoundingErrorWei);
        assertApproxEqAbs(siusd.previewRedeem(siusd.balanceOf(bob)), 212.5e18, maxRoundingErrorWei);

        assertApproxEqAbs(yieldSharing.vesting(), 5e18, maxRoundingErrorWei);
        assertApproxEqAbs(yieldSharing.vested(), 0, maxRoundingErrorWei);

        // go past the end of the interpolation window (125%)
        vm.warp(block.timestamp + interpolationDuration / 2);
        maxRoundingErrorWei += interpolationDuration / 2;
        yieldSharing.distributeInterpolationRewards();

        uint256 aliceEndBalance = siusd.previewRedeem(siusd.balanceOf(alice));
        uint256 bobEndBalance = siusd.previewRedeem(siusd.balanceOf(bob));
        assertApproxEqAbs(aliceEndBalance, 215e18, maxRoundingErrorWei);
        assertApproxEqAbs(bobEndBalance, 215e18, maxRoundingErrorWei);
        assertApproxEqAbs(aliceEndBalance + bobEndBalance, 430e18, 2);
    }

    function testMigrateSafetyBuffer() public {
        // mint 1000 iUSD
        usdc.mint(address(this), 500e6);
        usdc.approve(address(gateway), 500e6);
        gateway.mint(address(this), 500e6);

        assertEq(iusd.balanceOf(address(this)), 1000e18);
        assertEq(iusd.balanceOf(address(yieldSharing)), 0);

        // approve iusd for donation to the safety buffer
        iusd.approve(address(yieldSharing), 700e18);

        vm.expectRevert("UNAUTHORIZED");
        yieldSharing.migrateSafetyBuffer(address(this));

        vm.prank(governorAddress);
        yieldSharing.migrateSafetyBuffer(address(this));

        assertEq(iusd.balanceOf(address(this)), 300e18);
        assertEq(iusd.balanceOf(address(yieldSharing)), 700e18);
    }
}
