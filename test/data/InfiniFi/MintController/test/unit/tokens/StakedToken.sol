...
contract StakedTokenUnitTest is Fixture {
    ...
    function testUnstake() public {
        vm.startPrank(alice);
        siusd.approve(address(gateway), 1000e18);
        gateway.unstake(alice, 1000e18);
        vm.stopPrank();
        assertEq(siusd.balanceOf(address(alice)), 0);
    ...
    function testMaxGetters() public {
        ...
        vm.startPrank(carol);
        iusd.approve(address(gateway), amount);
        gateway.stake(carol, amount);
        vm.stopPrank();
